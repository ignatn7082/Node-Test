...
export default {
  ...
  data() {
    ...
    return {
      contactLocal: this.contact, // Liên kết với đối tượng contact để thao tác
      contactFormSchema, // Schema cho validation
    };
  },
  methods: {
    submitContact() {
    ...
    Cancel() {
      ...
      if (!reply) {
        return false; // Nếu người dùng chọn "Cancel", giữ trang hiện tại
      } else {
        this.$router.push({ name: "contactbook" }); // Trở lại danh sách liên hệ
      }
    },
  },
};
